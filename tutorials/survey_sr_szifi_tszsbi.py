"""
Same implementation as `cosmocnc_jax/surveys/survey_sr_planck_sim.py` but observable name
`q_szifi_tszsbi` (kept for older notebooks). Prefer the package **Planck_sim** survey module.
"""
import numpy as np
import jax
...
def sr_q_szifi_tszsbi_layer0(x0, prefactor_logy0, prefactor_M_500_to_theta,
                        sigma_sz_poly, alpha_szifi):
    """Pure JAX: q_szifi_tszsbi layer 0: lnM -> log(y0/sigma)."""
...
def sr_q_szifi_tszsbi_layer0_deriv(log_theta_500, sigma_sz_polyder, alpha_szifi):
    """Pure JAX: derivative of q_szifi_tszsbi layer 0."""
    return alpha_szifi - jnp.polyval(sigma_sz_polyder, log_theta_500) / 3.


def sr_q_szifi_tszsbi_layer1(x0, _pref_logy0, _pref_theta, dof):
    """Pure JAX: q_szifi_tszsbi layer 1: log(s/n) -> q = sqrt(exp(x)^2 + dof).
    First 2 args after x0 are prefactors (passed through, unused here)."""
    return jnp.sqrt(jnp.exp(x0)**2 + dof)


def sr_q_szifi_tszsbi_layer1_deriv(x0, dof):
    """Pure JAX: derivative of q_szifi_tszsbi layer 1."""
    exp2 = jnp.exp(2. * x0)
...
def precompute_q_prefactors(E_z, H0, D_A, A_szifi, bias_sz, alpha_szifi):
    """Pure JAX: precompute q_szifi_tszsbi prefactors at a single redshift."""
    h70 = H0 / 70.
...
class scaling_relations:
    ...
    def initialise_scaling_relation(self,cosmology=None):
        ...
        if observable == "q_szifi_tszsbi":
            # σ(θ): sky-averaged curve from tszsbi/noise_files only (same construction as
            # tszpower.maskedpower.compute_sigma_y0 — weighted mean over tiles with skyfracs).
            import os
            ...
            filter_name = self.cnc_params.get(
                "tszsbi_filter_name",
                os.environ.get("TSZSBI_FILTER_NAME", "immf6"),
            )
            ...
            theta_500_vec = np.exp(
                np.linspace(
                    np.log(theta_min_arcmin),
                    np.log(theta_max_arcmin),
                    ntheta,
                )
            )
    ...
    def precompute_scaling_relation(self,params=None,other_params=None,patch_index=0):
        ...
        elif observable == "q_szifi_tszsbi":
    ...
    def get_cutoff(self,layer=0):
        ...
        if self.observable == "q_szifi_tszsbi":
    ...
    def get_layer_fn(self, layer):
        ...
        """
        obs = self.observable
        if obs == "q_szifi_tszsbi":
            if layer == 0:
                return sr_q_szifi_tszsbi_layer0
            elif layer == 1:
                return sr_q_szifi_tszsbi_layer1
    ...
    def get_layer_returns_aux(self, layer):
        ...
        if self.observable == "q_szifi_tszsbi" and layer == 0:
            return True
        return False
    ...
    def get_layer_deriv_fn(self, layer):
        ...
        if obs == "q_szifi_tszsbi":
            if layer == 0:
                return sr_q_szifi_tszsbi_layer0_deriv
            elif layer == 1:
                return sr_q_szifi_tszsbi_layer1_deriv
        return None
    ...
    def get_layer_deriv_sr_params(self, layer, sr_params):
        ...
        For q_szifi_tszsbi layer 0: (sigma_sz_polyder, alpha_szifi)
        For q_szifi_tszsbi layer 1: (dof,)
        """
        obs = self.observable
        if obs == "q_szifi_tszsbi":
            if layer == 0:
...
class scatter:
    ...
    def get_cov(self,observable1=None,observable2=None,patch1=0,patch2=0,layer=0,other_params=None):
        ...
        if layer == 0:
            ...
            elif ((observable1 == "p_so_sim" and observable2 == "q_szifi_tszsbi") or
                  (observable1 == "q_szifi_tszsbi" and observable2 == "p_so_sim")):

                cov =  self.params["corr_lnq_lnp"]*self.params["sigma_lnq_szifi"]*self.params["sigma_lnp"]

            elif (observable1 == "q_szifi_tszsbi" and observable2 == "q_szifi_tszsbi"):
