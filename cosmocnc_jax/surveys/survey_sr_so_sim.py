...
def sr_q_so_sim_layer0(x0, prefactor_logy0, prefactor_M_500_to_theta,
                        sigma_sz_poly, alpha_szifi, beta_szifi):
...
def sr_q_so_sim_layer0_deriv(log_theta_500, sigma_sz_polyder, alpha_szifi, beta_szifi):
...
class scaling_relations:
    ...
    def precompute_scaling_relation(self,params=None,other_params=None,patch_index=0):
        ...
        if observable == "p_so_sim" or observable == "p_so_sim_stacked":
            ...
            self.prefactor_logy0 = jnp.log(10.**(A_szifi)*E_z**self.params["beta_szifi"]*(self.params["bias_sz"]/3.*h70)**self.params["alpha_szifi"]/jnp.sqrt(h70))
    ...
    def eval_scaling_relation_no_precompute(self,x0,layer=0,patch_index=0,other_params=None,params=None):
        ...
        if observable == "q_so_sim":
            ...
            if layer == 0:
                ...
                prefactor_logy0 = jnp.log(10.**(A_szifi)*E_z**self.params["beta_szifi"]*(self.params["bias_sz"]/3.*h70)**self.params["alpha_szifi"]/jnp.sqrt(h70))
    ...
    def get_layer_sr_params(self, layer, sr_params):
        ...
        if obs == "q_so_sim":
            if layer == 0:
                return (self.sigma_sz_poly, jnp.float64(sr_params["alpha_szifi"]), jnp.float64(sr_params["beta_szifi"]))
            elif layer == 1:
    ...
    def get_layer_deriv_sr_params(self, layer, sr_params):
        ...
        if obs == "q_so_sim":
            if layer == 0:
                return (self.sigma_sz_polyder, jnp.float64(sr_params["alpha_szifi"]), jnp.float64(sr_params["beta_szifi"]))
            elif layer == 1:
                return (jnp.float64(sr_params["dof"]),)
        return ()
