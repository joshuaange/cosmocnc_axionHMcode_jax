"""N-D backward conv tests — cosmocnc (NumPy) evaluations only.

Runs all cosmocnc evaluations and saves results to a pickle file.
No JAX imports — avoids classy_sz cross-contamination.
"""
...
RESULTS_FILE = os.path.join(os.path.dirname(__file__), "plots", "nd_results_cosmocnc.pkl")
...
def get_instance(mode, npts):
    ...
    if key in _pool:
        return _pool[key]
    print(f"  [init] cosmocnc {mode} n_pts_dl={npts}...")
    t0 = time.time()
...
def main():
    ...
    print("\n" + "=" * 60)
    print("SECTION 2: CONVERGENCE (cosmocnc)")
    print("=" * 60)
    ...
    # ── Section 4: Performance (cosmocnc at npts=128 only) ──
    print("\n" + "=" * 60)
    print("SECTION 4: PERFORMANCE (cosmocnc)")
    print("=" * 60)
    ...
    for mode in ["1d", "2d"]:
        ...
        perf[mode] = {"avg_ms": avg_ms, "min_ms": np.min(times) * 1000}
        print(f"  {mode}: avg={avg_ms:.0f}ms")
    results["perf"] = perf

    # ── Section 5: Accuracy (fiducial at each npts) ──
    print("\n" + "=" * 60)
    print("SECTION 5: ACCURACY (cosmocnc)")
    print("=" * 60)
